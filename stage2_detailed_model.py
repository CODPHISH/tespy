...
class CompleteBoilerTurbineModel:
    ...
    def build_network(self) -> Network:
        ...
        c_hp1_out = Connection(turb_hp1, "out1", turb_hp2, "in1", label="抽凝式汽轮机1_高压蒸汽去高压缸2段")
        ...
        # 【低压缸路径 - 7段，直接连接无抽汽】
        c_lp1_out = Connection(turb_lp1, "out1", turb_lp2, "in1", label="抽凝式汽轮机1_再热蒸汽去中压缸2段")
        c_lp2_out = Connection(turb_lp2, "out1", turb_lp3, "in1", label="抽凝式汽轮机1_再热蒸汽去中压缸3段")
        c_lp3_out = Connection(turb_lp3, "out1", turb_lp4, "in1", label="抽凝式汽轮机1_再热蒸汽去中压缸4段")
        c_lp4_out = Connection(turb_lp4, "out1", turb_lp5, "in1", label="抽凝式汽轮机1_再热蒸汽去中压缸5段")
        c_lp5_out = Connection(turb_lp5, "out1", turb_lp6, "in1", label="抽凝式汽轮机1_再热蒸汽去中压缸6段")
        c_lp6_out = Connection(turb_lp6, "out1", turb_lp7, "in1", label="抽凝式汽轮机1_再热蒸汽去中压缸7段")
    ...
    def _set_boundary_conditions(self, nw: Network) -> None:
        ...
        # 转炉煤气：固定流量、温度、流体组分（不设置压力，因为与高炉煤气压力线性相关）
        try:
        ...
        # 焦炉煤气：固定流量、温度、流体组分（不设置压力，因为与merge输出压力线性相关）
        try:
        ...
        # 高压缸排汽抽汽：固定流量（静态数据给出为20 t/h），不设置压力
        try:
