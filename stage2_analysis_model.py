...
class AnalysisBoilerTurbineModel:
    """基于参考数据分析的模型"""
    ...
    def load_reference_data(self) -> None:
        """加载参考数据（connections.csv）"""
        base_path = PROJECT_ROOT / "boiler-turbine_design_state"
        conn_file = base_path / "connections.csv"
        ...
        with open(conn_file, 'r', encoding='utf-8') as f:
            ...
            for i, row in enumerate(reader):
                if i >= 66:  # 限制行数
                    break
                label = row[''].strip() if row[''] else f"conn_{i}"
                try:
                    self.conn_data[label] = {
                        'm': float(row.get('m', 0)) if row.get('m', '0') != 'nan' else 0,
                        'p': float(row.get('p', 0)) if row.get('p', '0') != 'nan' else 0,
                        'T': float(row.get('T', 0)) if row.get('T', '0') != 'nan' else 0,
                    }
                except:
                    pass
        
        print(f"✓ 加载了 {len(self.conn_data)} 条连接参考数据")

    def build_network(self) -> Network:
        """构建网络"""
        print("\n" + "=" * 80)
        print("分析模型 - 基于参考数据的最小约束")
        print("=" * 80)
    ...
    def _set_boundary_conditions(self, nw: Network) -> None:
        ...
        try:
            ...
            conn.set_attr(p=1.0, T=99.0, fluid={'water': 1})  # 简化条件
        except KeyError:
            pass
        ...
        try:
            ...
            conn.set_attr(m=500, T=20, fluid={'N2': 0.76, 'O2': 0.24})
        except KeyError:
            pass
        ...
        for label, fluid_comp in fuel_inlets:
            try:
                ...
                conn.set_attr(m=50, T=20, fluid=fluid_comp)
            except KeyError:
                pass
    ...
    def _set_component_parameters(self, nw: Network) -> None:
        ...
        # 汽轮机效率
        turbine_eta = [
            ("抽凝式汽轮机1_高压缸一段", 0.7475),
            ("抽凝式汽轮机1_高压缸二段", 0.7475),
            ("抽凝式汽轮机1_低压缸一段", 0.7473),
            ("抽凝式汽轮机1_低压缸二段", 0.7473),
            ("抽凝式汽轮机1_低压缸三段", 0.7473),
            ("抽凝式汽轮机1_低压缸四段", 0.7473),
            ("抽凝式汽轮机1_低压缸五段", 0.7473),
            ("抽凝式汽轮机1_低压缸六段", 0.7473),
            ("抽凝式汽轮机1_低压缸七段", 0.7473),
        ]

        for name, eta_s in turbine_eta:
            try:
...
def main() -> None:
    ...
    model = AnalysisBoilerTurbineModel()
    model.build_network()
