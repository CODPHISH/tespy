...
基于参考数据，阶段2正在开发一个精细化的锅炉-汽轮机联合循环模型：

| 文件 | 功能 | 状态 |
|------|------|------|
| `stage2_detailed_model.py` | 多级汽轮机 + 多级换热器的详细建模示例 | 🔄 开发中 |

### 模型设计

- **汽轮机系统**：高压缸2段 + 低压缸7段，基于实际电厂结构
- **锅炉系统**：下省煤器 → 上省煤器 → 蒸发器 → 低温过热器 → 屏式过热器 → 三级过热器 → 末级过热器
- **再热系统**：低温再热器 → 末级再热器
- **参数来源**：基于参考数据中的真实电厂设计工况（161 bar / 560°C，287.6 t/h）

### 当前状态

模型结构已按照参考数据完整搭建（21个组件，31个连接），包含：
- 9段汽轮机（与参考数据一致的pr和eta_s）
- 9个换热器（与参考数据一致的kA值）
- 完整的烟气-蒸汽-水三侧连接

**注意**：由于约束参数繁多，该模型当前处于调试阶段，尚未实现稳定收敛。建议先使用 `stage2_minimal_working.py` 进行基础学习。
